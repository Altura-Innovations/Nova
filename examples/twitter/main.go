...
import (
	...
	"github.com/Neura-AI-Labs/nova/internal/twitter"
...
func main() {
	...
	k, err := twitter.New(
		...
		twitter.WithLogger(log.NewSubLogger("nova", &logger.SubLoggerOpts{})),
	...
	if err != nil {
		log.Fatalf("Failed to create nova: %v", err)
	}

	// Start nova
	if err := k.Start(); err != nil {
		log.Fatalf("Failed to start nova: %v", err)
	}

	// Wait for interrupt signal
	<-ctx.Done()

	// Stop nova gracefully
	if err := k.Stop(); err != nil {
		log.Errorf("Error stopping nova: %v", err)
	}
}
